...
const getDefaultAudioConfig = () => ({
  ...
  >,
  asrProvidersConfig: {
    'xiaomi-mimo-asr': { apiKey: '', baseUrl: '', enabled: true },
        ...
        fetchServerProviders: async () => {
          try {
            ...
            set((state) => {
              ...
              const currentTtsWasUsable = isProviderUsable(
                state.ttsProvidersConfig[state.ttsProviderId],
              );

              // A browser that previously had no usable TTS provider may have
              // persisted the silent default. Enable speech once when a newly
              // managed provider replaces that unusable selection. Once the
              // managed provider is persisted, a later user opt-out is kept.
              if (!state.ttsEnabled && !currentTtsWasUsable && serverTtsIds.length > 0) {
                autoTtsEnabled = true;
              }
