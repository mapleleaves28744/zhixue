...
export function resolveClassroomTTSConcurrency(value: string | undefined): number {
...
export async function generateTTSForClassroom(
  ...
  const speechTasks: Array<{ sceneOrder: number; action: SpeechAction }> = [];
  for (const scene of scenes) {
    ...
    for (const action of scene.actions) {
      ...
      speechTasks.push({ sceneOrder: scene.order, action: action as SpeechAction });
    }
  }

  const concurrency = resolveClassroomTTSConcurrency(process.env.OPENMAIC_CLASSROOM_TTS_CONCURRENCY);
  await mapWithConcurrency(speechTasks, concurrency, async ({ sceneOrder, action: speechAction }) => {
      // Include scene order in audioId to prevent collision across scenes
      const audioId = `tts_s${sceneOrder}_${speechAction.id}`;
