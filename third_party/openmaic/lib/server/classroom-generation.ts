...
const DEFAULT_SCENE_STEP_TIMEOUT_MS = 3 * 60 * 1000;

export function resolveClassroomSceneConcurrency(value: string | undefined): number {
  const parsed = Number.parseInt(value ?? '', 10);
  if (!Number.isFinite(parsed)) return 2;
  return Math.max(1, Math.min(parsed, 4));
}

export async function runClassroomGenerationStep<T>(
  label: string,
  timeoutMs: number,
  operation: () => Promise<T>,
): Promise<T> {
...
export async function generateClassroom(
  ...
  const sceneStepTimeoutMs = Math.max(
    10_000,
    Number(process.env.OPENMAIC_SCENE_STEP_TIMEOUT_MS) || DEFAULT_SCENE_STEP_TIMEOUT_MS,
  );
  const sceneConcurrency = resolveClassroomSceneConcurrency(
    process.env.OPENMAIC_CLASSROOM_SCENE_CONCURRENCY ?? process.env.PARALLEL_SCENE_CONCURRENCY,
  );

  const sceneDrafts = lazyBoundedMap(outlines, sceneConcurrency, async (outline) => {
    const safeOutline = applyOutlineFallbacks(outline, true, { allowProceduralSkill: vocationalActive });
    try {
      const content = await runClassroomGenerationStep(
        `Scene "${safeOutline.title}" content generation`,
        sceneStepTimeoutMs,
        () =>
      ...
      const actions = await runClassroomGenerationStep(
        `Scene "${safeOutline.title}" action generation`,
        sceneStepTimeoutMs,
        () =>
      ...
      log.warn(`Skipping scene "${safeOutline.title}" after generation error:`, error);
      return undefined;
    }
  });

  for (const [index, draftPromise] of sceneDrafts.entries()) {
    ...
    await options.onProgress?.({
      ...
      message: `Generating scene ${index + 1}/${outlines.length}: ${outline.title} (concurrency ${sceneConcurrency})`,
    ...
    if (draft) {
      log.info(`Scene "${draft.safeOutline.title}": ${draft.actions.length} actions`);
      const sceneId = createSceneWithActions(draft.safeOutline, draft.content, draft.actions, api);
      if (!sceneId) {
        log.warn(`Skipping scene "${draft.safeOutline.title}" — scene creation failed`);
      } else {
        generatedScenes += 1;
      }
    }
    ...
    await options.onProgress?.({
      ...
      message: `Processed ${index + 1}/${outlines.length} scenes (${generatedScenes} generated)`,
      scenesGenerated: generatedScenes,
      totalScenes: outlines.length,
    });
  }
