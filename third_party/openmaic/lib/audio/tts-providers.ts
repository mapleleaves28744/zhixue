...
export async function generateTTS(
  ...
  switch (config.providerId) {
    case 'xiaomi-mimo-tts':
      return await generateXiaomiMiMoTTS(config, text);
...
/**
 * Xiaomi MiMo TTS uses OpenAI-compatible chat completions with an audio response.
 */
async function generateXiaomiMiMoTTS(
  config: TTSModelConfig,
  text: string,
): Promise<TTSGenerationResult> {
  const baseUrl = (config.baseUrl || TTS_PROVIDERS['xiaomi-mimo-tts'].defaultBaseUrl || '').replace(
    /\/$/,
    '',
  );
  ...
  if (!audioBase64) {
    throw new Error('Xiaomi MiMo TTS API error: no audio data returned');
  }
  return {
