...
export async function transcribeAudio(
  ...
  switch (config.providerId) {
    case 'xiaomi-mimo-asr':
      return await transcribeXiaomiMiMoASR(config, audioBuffer);
...
/**
 * Xiaomi MiMo ASR uses OpenAI-compatible chat completions with input_audio content.
 */
async function transcribeXiaomiMiMoASR(
  config: ASRModelConfig,
  audioBuffer: Buffer | Blob,
): Promise<ASRTranscriptionResult> {
  const baseUrl = (config.baseUrl || ASR_PROVIDERS['xiaomi-mimo-asr'].defaultBaseUrl || '').replace(
    /\/$/,
    '',
  );
  ...
  const response = await fetch(`${baseUrl}/chat/completions`, {
    ...
    body: JSON.stringify({
      model: config.modelId || 'mimo-v2.5-asr',
      messages: [
        {
          role: 'user',
          content: [
