...
import {
  ...
  resolveClassroomTTSConcurrency,
...
describe('classroom TTS concurrency', () => {
  test('uses a conservative parallel default and clamps overrides', () => {
    expect(resolveClassroomTTSConcurrency(undefined)).toBe(2);
    expect(resolveClassroomTTSConcurrency('1')).toBe(1);
    expect(resolveClassroomTTSConcurrency('99')).toBe(4);
  });
});
