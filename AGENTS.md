...
维护当前项目、判断接口是否存在或判断功能是否完成时，必须先阅读：

```text
docs/当前实现基线.md
docs/00_文档规范/04_文档事实源与状态规则.md
docs/当前实现API清单.md
docs/当前实现数据库清单.md
```
...
早期设计方案用于表达目标和约束，不代表所有页面、接口、表或组件已经实现。若设计与实现冲突：

1. 不得按设计文档虚构当前能力；
2. 先记录差异；
3. 当前任务是维护/修复时，以代码事实为准并同步文档；
4. 当前任务是新增能力时，遵守设计边界并明确是否需要调整设计。

修改 Router、Schema 或 SQLAlchemy Model 后必须执行：
...
1. `docs/00_文档规范` 至 `docs/24_答辩稿` 每个编号目录必须保留一份与目录同名的总文档，并以“目录导读”开头。
2. 不要再为待写主题创建只有标题和“待补充”的占位文档；内容应直接补入目录总文档。
3. 只有自动生成事实清单、验收记录、运行指南等需要独立持续维护的内容才新增独立文档。
4. 文档变更后执行 `python scripts/check_docs.py`，确保没有占位模板和本地断链。
...
8. 代码必须可读、可测试、可扩展。
9. 新增业务功能必须遵守 `docs/` 中的设计边界，并同步当前实现文档。
10. 维护已有功能时以代码、migration、OpenAPI 和当前实现基线为事实；发现设计冲突必须说明并同步文档。
...
4. 类型定义必须放入 `types/`。
5. 全局状态放入 `stores/`。
6. 页面应遵守 `docs/13_UI设计规范/13_UI设计规范.md`。
7. 不要做普通后台模板风格。
...
```text
docs/11_API接口设计/11_API接口设计.md
docs/10_数据库设计/10_数据库设计.md
docs/12_前端页面设计/12_前端页面设计.md
```
...
| 任务类型 | 必读文档 |
|---|---|
| PRD / 需求 | `docs/03_PRD产品需求文档/03_PRD产品需求文档.md` |
| 系统架构 | `docs/06_系统架构设计/06_系统架构设计.md` |
| API | `docs/11_API接口设计/11_API接口设计.md` |
| 数据库 | `docs/10_数据库设计/10_数据库设计.md` |
| 前端 | `docs/12_前端页面设计/12_前端页面设计.md`、`docs/13_UI设计规范/13_UI设计规范.md` |
| Wiki | `docs/09_LLM_Wiki学习空间设计/09_LLM_Wiki学习空间设计.md` |
| Agent | `docs/07_多智能体架构设计/07_多智能体架构设计.md` |
| 自进化 | `docs/08_自进化学习智能体设计/08_自进化学习智能体设计.md` |
| 部署 | `docs/20_部署方案/20_部署方案.md` |
