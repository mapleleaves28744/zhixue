...
2. 规则层 `_apply_safety_net` 仅在必交付物缺失、显式引用约束、用户 `tool_hints` 或 Mock 空转时介入。
...
- 个性化学习闭环已完成第一版工程化修复：长期记忆使用稳定 `memory_key` 合并强化、反思水位仅处理新行为，课程作用域最多保留 20 条活跃记忆、全局最多 10 条，超限与历史重复记录只归档不删除；Tutor、Resource 与个性化上下文只加载当前课程最相关的 5 条活跃记忆。
...
- **OpenMAIC 个性化沉浸课堂**：`/assistant` 可将“一句话生成沉浸课堂”路由到 `generate_immersive_classroom`，基于课程 RAG 引用、学生画像、薄弱点和讲解偏好创建后台任务；完成后展示可启动的课堂卡片，并继续异步导出带配音与烧录字幕的 MP4。
...
| 验收项 | 最新可引用数字 | 来源 |
|---|---:|---|
| 后端全量测试 | **`322 passed`**（2026-06-13 本地回归） | `cd backend && python -m pytest -q` |
...
| 个性化闭环 | **322 passed**（含 `test_personalization_closed_loop.py` 等） | [`../19_测试方案/17_个性化学习闭环专项验收记录.md`](../19_测试方案/17_个性化学习闭环专项验收记录.md) |
| 资源工具矩阵 | 13 种直接资源类型 | [`../19_测试方案/22_智能对话资源生成工具专项验收记录.md`](../19_测试方案/22_智能对话资源生成工具专项验收记录.md) |
...
| 资源工具矩阵 | 13 种资源类型、侧栏、Agent tool_hints | 资源工具专项 + 浏览器 | 22_资源工具验收 |
