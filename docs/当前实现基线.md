...
> AI 阅读边界：本文只描述当前代码事实和已有验收证据。`docs/superpowers/` 与 `docs/_archive/` 不代表已实现；[全面代码评估问题修复计划](19_测试方案/26_全面代码评估问题修复计划.md) 是待执行计划，不得据此宣称问题已经修复。
...
| Next.js 页面路由 | 11 |
| Stitch HTML 页面 | 8 |
| 后端 pytest | 503 passed |
| 真实 LLM 主链路 | 23 步通过 |
| 前端依赖审计 | 0 vulnerabilities（最近证据：2026-07-13；2026-07-24 未复跑） |
...
- **Agent Runtime 收敛（2026-07-12～13，`change_8`）**：Supervisor 对可识别意图仅提供对应的候选工具（无候选时保留全量回退）；模型请求非候选工具时会改用安全候选或进入 `replan`，不会越过 Tool Registry；工具处理失败先回滚会话再持久化失败结果；Worker 以条件更新原子认领 `queued` 任务，并补齐取消竞态与事务边界回归。此次收敛未新增数据库表或 API。
...
- **知识图谱掌握度（2026-07-14）**：未产生有效学习证据的知识点以 50% 中性先验和低置信度展示为“待验证”，不再误标为薄弱；答题采用随有效证据次数递减的加权更新，单纯提问不扣分；遗忘在首日宽限后缓慢回落至 35% 中性下限，并在 `evidence_json` 记录置信度和有效证据次数。
- **学生画像掌握度摘要（2026-07-14）**：`/path-profile` 仅采信带 `evidence_weighted_v2` 版本、有效证据数和置信度的课程画像掌握度快照；旧快照不会再以 1% 等历史值覆盖当前规则。无证据时圆环显示 50% 与“待验证”，后续随有效学习证据动态更新。
- **课程知识库质量与 Wiki 质量（2026-07-14）**：`GET /api/v1/knowledge/seed-quality-report?course_id=...` 会按选中课程实时汇总资料、解析、切片、知识点、Wiki 来源与页面关系，不再依赖生产容器中可选的种子报告文件；Wiki 列表/详情返回 `content_quality`，将有来源且正文结构完整的页面标为“已核验”并优先展示，历史无来源或正文过短页面明确标为“待补强”。学习仪表盘知识图谱节点会深链至对应 Wiki 详情或图谱节点详情。
- **待补强 Wiki 逐页重建（2026-07-14）**：新增 `scripts/rebuild_low_quality_wiki.py` 与 `WikiGenerateService.rebuild_low_quality_pages()`，按页面串行使用课程资料切片补强旧 Wiki；每页单独创建新版本、绑定知识点/切片来源并通过内容质量规则后才标为“已核验”。当前演示课程已补强 17 页，质量统计为 36 个已核验、13 个待补充、16 个待补强；剩余待补强页没有可验证的关联课程资料，保留原版本而不使用无关内容填充。
...
- **Agent Runtime Provider 边界**：`AgentRuntimeService` 当前以 `allow_mock_fallback=False` 获取 Provider，因此 `/assistant` 智能体模式需要真实 LLM Provider；普通 Tutor、Wiki、资源等支持 Mock，不等于 LangGraph Agent Runtime 在无 Key 时可完整运行。
...
- **60 秒讲解视频**：本地视频渲染器以 6 段教学分镜导出 1280×720 MP4，每段包含中文标题、讲解正文、关键点卡片、进度条和烧录字幕；优先合成 MiMo TTS 音轨，失败时保留带字幕的静音 MP4。生产镜像显式安装并缓存 Noto CJK 字体，避免中文缺字或方块字。
- **知识卡片**：`generate_educational_image` 在 Mock/无 Key 时自动降级为简明 Mermaid 知识卡片；已支持 Qwen Image、CloudBase、Agnes 等文生图 Provider。其中 Qwen Image 使用用户自然语言需求与官方自动提示词扩展，避免追加冗长硬性模板；需要精确节点或关系的图继续使用 Mermaid。
...
- **练习推荐**：`PracticeSuggestionService` 根据 `/assistant` 最近用户提问抽取知识点，经 `/api/v1/quizzes/suggest-topics` 返回推荐练习主题；`PracticePrepushService` 支持预推送练习入口。
- **联网搜索**：Agent 工具 `search_web` 经 `WebSearchService` 调用 AnySearch（`third_party/anysearch-skill`），返回可引用网页摘要。
- **HTML PPT 课件**：Agent 工具 `generate_interactive_courseware` 基于 `html-ppt-skill` 生成固定 7 页的课程叙事（封面、目标、概念、例子、练习、自测、总结）；模型输出会按页型、文本长度和卡片数量归一化，避免长指令或重复内容撑爆页面。
- **外部资源推荐**：`ExternalResourceFeedService` 聚合外部学习资源 feed，供推荐与助手侧栏使用。
- **个性化推荐与实时诊断增强**（2026-06-14）：推荐链路、诊断刷新与 Stitch 页面体验修复，详见 `git log 8304a17`。
...
- `/courses` 的真实课程面板与 CRUD 已接入 API，但原 Stitch Hero 仍显示 `45% / 24 份 / 8 人` 固定指标，“导入示例数据”按钮也只提示后续接入；录屏和答辩不得把这些值解释为实时统计。
...
- 真实 Provider **全量**验收基线尚未建立：现有 `scripts/real_provider_acceptance.py` 只有 Provider 分类、fallback 拒绝和错误脱敏辅助函数，没有可执行全场景 CLI、逐场景延迟或产物矩阵；2026-06-06 的 23 步真实 LLM 主链路仍是独立有效证据，但不能替代文本、图像、音频、视频与沉浸课堂的全量 Provider 验收。
- MinIO 存储。
...
以下问题尚未修复，不得在文档或答辩中描述为已经闭环。详细任务、影响、测试和回滚见 [26_全面代码评估问题修复计划](19_测试方案/26_全面代码评估问题修复计划.md)。

| 风险 | 当前代码事实 | 状态 |
|---|---|---|
| Agent Review 未形成阻断分支 | `graph.py` 当前固定 `review → memory_reflect → finalize`，`review_result` 未决定后续路由 | 已静态确认，待 T06 修复 |
| 写库工具重试副作用 | Tool Registry 会按重试策略重新调用 handler，成功缓存无法覆盖“业务已写入、结果未保存”的窗口 | 已静态确认，待 T07/T08 修复 |
| 公共课程资料写权限 | `MaterialService.upload_material()` 当前使用课程可读权限检查 | 已静态确认，待 T03 修复 |
| Agent 无 Key Mock 边界 | Agent Runtime 明确禁止 Mock fallback | 已静态确认，待 T09 修复 |
| 知识搜索前后端契约 | 后端返回 `{items, graph_context}`，部分 Stitch/TypeScript 调用按数组处理 | 已静态确认，待 T05 修复 |
| 健康检查过于乐观 | `/health` 不检查 DB/Redis/pgvector；部分启动异常被忽略 | 已静态确认，待 T12 修复 |
| 资料处理阻塞与部分成功 | 同步解析/本地 Embedding 位于 async 链路；自动切片异常可能被吞掉 | 已静态确认，待 T10/T11 修复 |
| Token 与 API Base | 媒体仍使用查询 JWT；前端允许 URL 覆盖任意 API Base；Token 存储边界仍需收紧 | 已静态确认，待 T02/T18/T20/T21 修复 |
| Agent SSE 一致性 | 历史回放与实时订阅存在事件窗口，长连接 Session 生命周期待专项验证 | 部分已知、部分待复现，待 T14/T15 |
| 引用质量 | Mock 评测引用精度与覆盖率偏低，当前校验主要检查引用标记而非主张支撑 | 已有评测证据，待 T26/T27 |
