...
| `/assistant` | `assistant.html`，统一 Agent 对话入口、动态计划、工具调用、观察/重规划、引用与产物时间线，可通过自然对话沉淀学习画像 |
...
- A/B 测试框架已实现：`models/ab_test.py`（实验定义 + 用户分组）、`services/ab_test_service.py`（确定性分组 + 自动胜出判定）、`api/v1/ab_tests.py`（7 个端点）。
