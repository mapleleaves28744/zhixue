...
| Sprint | 内容 | 状态 |
|--------|------|------|
| Sprint 1 | 新建 `stitch-shared.css` + `zhixue-ui.js`，迁移 IP 贴纸到 `public/stickers`，8 页引用共享层并删除重复 style | 待实施 |
| Sprint 2a | 按优先级改造 8 个 Stitch HTML（品牌页→dashboard→knowledge→assistant→practice→path-profile→courses→home） | 待实施 |
| Sprint 2b | 扩展 `zhixue-static-api.js`（skeleton、toast、401、getPageMascot） | 待实施 |
| Sprint 3 | CSS 微动效 + scrollReveal；StitchFrame 安装 framer-motion；AI/Agent 状态动效 | 待实施 |
...
### Sprint 2：全站 8 页视觉与信息层级升级

按 **演示价值 × 改动量** 排序实施（每页 checklist 相同）：
