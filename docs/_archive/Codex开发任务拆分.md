...
> 文档状态：**历史任务规划与验收要求**
> 任务是否已经完成不能仅依据本文复选状态判断；当前完成度以 `docs/当前实现基线.md`、`docs/功能完成度与待完善清单.md` 和实际验收记录为准。

文档版本：V1.0  
适用位置：`docs/_archive/Codex开发任务拆分.md`  
...
6. 当前重新推进阶段时冻结教师端和管理员端新增页面，不再创建 `/teacher/*`、新增 `/admin/*` 页面或管理员演示初始化入口；比赛展示优先保证学生端主链路。
---
# 1. 阶段总览
| 阶段 | 目标 | 主要优先级 |
|---|---|---|
...
| 第17阶段：教师端（冻结，不建设） | 仅保留文档说明，不新增代码 | P0:0 / P1:0 / P2:0 |
| 第18阶段：管理员端（冻结，不新增） | 仅保留已有代码，不新增页面/API | P0:0 / P1:0 / P2:0 |
...
| Agent 日志 | `/path-profile`、`/dashboard` | 调用 `/api/v1/agents/runs` 展示调用链摘要 |

每个后端阶段完成后，必须同步检查对应 Stitch 页面是否已经接入真实 API。若未接入，不得宣称“阶段已完成”，只能说明“后端实现完成，前端 Stitch 联动未完成”。

## 1.3 当前重新推进范围冻结：不建设教师端和管理员端

从 2026-05-25 的修复推进开始，教师端和管理员端新增建设冻结：

1. 不创建 `frontend/app/teacher/*`、`frontend/app/admin/*` 新页面；
2. 不新增教师端 API、管理员端 API、管理员演示数据初始化入口；
3. 不恢复历史未提交代码中由 Claude Code 批量生成的教师端、管理员端页面；
4. 已有基础鉴权、`require_admin`、已有 `admin.py` 如被当前代码依赖可保留，但不作为第17/18阶段继续扩展目标；
5. 原第17阶段和第18阶段视为“范围冻结/跳过”，推进到第20阶段时重点落在第16阶段诊断推荐、第19阶段学生端 Stitch 接入与第20阶段测试。
...
> 当前状态：冻结并跳过。重新推进项目时不再建设教师端，不创建 `/teacher/*` 路由、页面、布局或教师端 API。教师相关能力仅在答辩和文档中说明为后续扩展方向，当前比赛演示聚焦学生端个性化学习闭环。
...
> 当前状态：冻结并跳过。重新推进项目时不再新增管理员端页面、管理员端 API 或演示初始化入口。已有基础管理相关代码如未影响主链路可保留，但第18阶段不再作为当前目标，避免偏离学生端主链路和引入额外风险。
