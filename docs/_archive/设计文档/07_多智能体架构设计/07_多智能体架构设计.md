...
> 本文是 `07_多智能体架构设计` 的汇总入口。正文包含历史设计与目标方案；判断当前实现时，以 `docs/当前实现基线.md`、当前代码和本目录保留的事实清单为准。
...
> 文档状态：**设计约束 + 部分增强规划**
> 当前已实现 14 个任务型 Agent（含 IntentRouter 与 Review），实际类、注册和调用关系以 `backend/app/agents`、`docs/当前实现基线.md` 和 Agent 运行日志为准。本文中的复杂协作流程不代表都已接入。

文档版本：V1.0  
适用位置：`docs/07_多智能体架构设计/07_多智能体架构设计.md`
...
| generated_resources | 读取 |
| learning_paths | 读取 |
| learning_path_items | 读取 |
| mastery_records | 读取 |
| student_profiles | 读取 |
| evolution_strategies | 读取 |
| wiki_pages | 读取 |
...
|---|---|
| evolution_events | 写入 |
| evolution_strategies | 读取 / 写入 |
| learning_preferences | 按需写入 Prompt 参数 |
| wiki_links | 按需写入 Wiki 结构关系 |
| student_profiles | 读取 |
| learning_memories | 读取 |
| diagnosis_reports | 读取 |
| behavior_events | 读取 |
...
|---|---:|---:|---:|---|
| Orchestrator | 否 | 否 | 间接 | agent_runs |
| Profile | 是 | 否 | 是 | student_profiles |
| Memory | 间接 | 否 | 是 | learning_memories |
| Wiki | 否 | 是 | 是 | wiki_pages, wiki_page_versions |
| Knowledge | 否 | 间接 | 间接 | knowledge_points, wiki_links |
| Planner | 否 | 否 | 是 | learning_paths |
| Resource | 间接 | 可选 | 是 | generated_resources |
| Quiz | 否 | 否 | 间接 | questions |
| Tutor | 间接 | 可选 | 是 | learning_records, wiki_sources |
| Diagnosis | 间接 | 间接 | 是 | diagnosis_reports, mastery_records |
| Recommend | 否 | 否 | 是 | recommendations |
| Evolution | 否 | 间接 | 是 | evolution_strategies, evolution_events |
| Review | 否 | 否 | 是 | agent_runs |
...
| status | 执行状态 |
| input_payload | 输入 |
| output_payload | 最终输出和关键步骤摘要 |
| started_at | 开始时间 |
| finished_at | 结束时间 |

MVP 阶段只落 `agent_runs` 和 `llm_call_logs` 两张表。Agent 步骤、工具调用、审核结果先写入 `agent_runs.output_payload` 的结构化字段中，避免数据库模型过早膨胀。增强版如需更细粒度追踪，再拆分 `agent_steps` 和 `tool_call_logs`。
