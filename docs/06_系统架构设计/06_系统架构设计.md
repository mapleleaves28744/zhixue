...
> 当前实现请先读取 `docs/当前实现基线.md`。当前没有管理员后台业务模块、MinIO Adapter、Redis 业务使用、GraphRAG/Rerank 或多厂商专用 Adapter；前端当前为 Next.js + Stitch iframe 形态。
...
| 层 | 当前实现 |
|---|---|
| 前端 | Next.js 16 App Router + `StitchFrame` + 8 个 Stitch HTML；主要业务请求由静态页面脚本发起 |
| 后端 | FastAPI 模块化单体，Router → Service → Repository → Model |
| 数据 | PostgreSQL + pgvector，28 张 ORM 表，16 个 migration |
| 缓存 | Redis URL 已配置，当前业务尚未接入 Redis |
| 文件 | 本地文件存储，当前无 MinIO Adapter |
| AI | Mock + OpenAI-compatible Adapter；聊天与 Embedding 独立配置 |
| Agent | 13 个任务型 Agent，统一 Context/Result，运行日志落库 |
