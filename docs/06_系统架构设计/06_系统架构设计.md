...
> 本文是 `06_系统架构设计` 的汇总入口。正文包含历史设计与目标方案；判断当前实现时，以 `docs/当前实现基线.md`、当前代码和本目录保留的事实清单为准。
...
> 文档状态：**目标架构与设计约束，非完整当前实现清单**
> 当前实现请先读取 `docs/当前实现基线.md`。当前没有管理员后台页面、MinIO Adapter、完整 GraphRAG 或多厂商专用 Adapter；Redis 已用于 arq 队列、Agent 事件和画像缓存；前端为 Next.js React 页面与 StitchFrame 混合形态。
...
| 层 | 当前实现 |
|---|---|
| 前端 | Next.js 16 App Router；`/`、`/assistant`、`/evolution` 为 React，其余核心页主要使用 `StitchFrame` |
| 后端 | FastAPI 模块化单体，Router → Service → Repository → Model |
| 数据 | PostgreSQL + pgvector，44 张 ORM 表，migration 以当前清单为准 |
| 缓存 | Redis 用于 arq 队列、实时事件通知和画像缓存；Redis 3.0 时回退 Pub/Sub 兼容模式 |
| 文件 | 本地文件存储，当前无 MinIO Adapter |
| AI | Mock + OpenAI-compatible Adapter；聊天与 Embedding 独立配置 |
| Agent | 14 个 Agent，LangGraph + MiMo Supervisor 动态编排，统一 Context/Result 与持久事件 |
| 部署 | 本地优先；Docker Compose 尚未完成正式全栈验收 |

本文后续内容包含目标架构和增强版设计。涉及管理员后台、多厂商专用 Adapter、MinIO、Rerank、GraphRAG、复杂异步任务的描述均为规划参考。
...
erDiagram
    ...
    answer_records ||--o{ mistake_books : produces
    ...
    users ||--o{ evolution_events : triggers
...
学生上传文件
  ...
  → 写入 wiki_pages / wiki_sources / wiki_links
  → 前端显示解析完成
```
...
| Agent 执行不可调试 | 出错难定位 | 记录 agent_runs、llm_call_logs，并在 agent_runs.output_payload 中保留关键步骤 |
