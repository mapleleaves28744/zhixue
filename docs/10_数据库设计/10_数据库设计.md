...
> 本文是 `10_数据库设计` 的汇总入口。正文包含历史设计与目标方案；判断当前实现时，以 `docs/当前实现基线.md`、当前代码和本目录保留的事实清单为准。
...
> 文档状态：**数据库设计意图与约束**
> 当前实际表、字段和外键以 `15_当前实现数据库清单.md`、Alembic migration 和 SQLAlchemy Model 为准。本文中的字段若未进入当前清单，视为规划项。

文档版本：V1.0  
适用位置：`docs/10_数据库设计/10_数据库设计.md`
