...
attribution: OpenDSA
imported_at: 2026-06-06T17:44:45.377102+00:00
text_hash: 165bf58ebb66dcb3b1f82ad0c49b363e127ddb5a8010e7cf10c2b666ab252a6a
---

# opendsa hashing introduction

14.1. Introduction — OpenDSA Complete Catalog 
...
| About « 13. 5. External Sorting :: Contents :: 14. 2. Hash Function Principles » 
...
Hashing works by performing a computation on a search key K in a way that is intended to identify the position in HT that contains the record with key K .
The function that does this calculation is called the hash function ,
...
for any key value K and some hash function \(h\) , \(i = \mathbf{h}(K)\) is a slot in the table such that \(0 <= i < M\) ,
...
if any, has key value K ?’ For applications where all search is done by exact-match queries,
hashing is the search method of choice because it is extremely
efficient when implemented correctly. As this tutorial shows, however, there are many approaches
...
the range 0 to \(n-1\) .
A record with key k can be stored in HT[k] , and so the hash function is \(\mathbf{h}(k) = k\) .
To find the record with key value k , look in HT[k] . 
...
Given a hash function h and two keys \(k_1\) and \(k_2\) , if \(\mathbf{h}(k_1) = \beta = \mathbf{h}(k_2)\) where \(\beta\) is a slot in
the table, then we say that \(k_1\) and \(k_2\) have a collision at slot \(\beta\) under hash function h . 
...
Starting with slot \(\mathbf{h}(K)\) , locate the record
containing key K using (if necessary) a collision resolution policy . 

Contact Us | | Privacy | | License « 13. 5. External Sorting :: Contents :: 14. 2. Hash Function Principles » 
